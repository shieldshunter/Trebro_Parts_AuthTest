...
`)),t.appendChild(c),this.hide()}display(t){this.loadFileCallback=t,this.modal.style.setProperty("pointer-events","auto"),this.modal.style.setProperty("display","block")}hide(){this.modal.style.setProperty("pointer-events","none"),this.modal.style.setProperty("display","none")}loadFile(t,a){this.hide(),w=!0,this.loadFileCallback&&this.loadFileCallback(t,a)}}customElements.define("drop-zone",k);async function b(){const i="https://partsmanual.blob.core.windows.net/authenticationhash/credentials.json?sp=r&st=2025-02-22T04:11:14Z&se=2025-03-08T12:11:14Z&sv=2022-11-02&sr=b&sig=lbb2TljS4yUSeTo8iDYcCmQgeZVq3uu7d90h6uqg4jM%3D";try{const e=await fetch(i);if(!e.ok)throw new Error(`Failed to fetch auth data: ${e.statusText}`);const t=await e.json(),a={};return t.forEach(o=>{a[o.Email.toLowerCase()]=o.Password}),console.log("Fetched authentication data:",a),a}catch(e){return console.error("Error fetching authentication data:",e),{}}}let h={};(async()=>h=await b())();async function C(){return h=await b(),h}async function L(i){const e=new TextEncoder().encode(i),t=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(t)).map(o=>o.toString(16).padStart(2,"0")).join("")}class T{async isAuthenticated(){const e=await this.getUserData();return!e||!e.email?!1:h[e.email.toLowerCase()]===e.hashedPassword||!1}async getUserData(){const{zeaUserData:e}=window.localStorage;return e?JSON.parse(e):null}async setUserData(e){{const t=e.email.toLowerCase();if(!h.hasOwnProperty(t))throw new Error("Email not registered. Please request access.");if(!e.password)throw new Error("Password not provided.");console.log("Original userData:",e);const a=await L(e.password);if(console.log("Computed hash:",a),console.log("Stored hash:",h[t]),h[t]!==a)throw new Error("Wrong password.");e.hashedPassword=a,e.password="".padEnd(6,"*")}console.log("Setting userData:",e),window.localStorage.zeaUserData=JSON.stringify(e)}async signOut(){console.log("Signing out"),localStorage.removeItem("zeaUserData")}}const g=new T;class A extends HTMLElement{constructor(){super();p(this,"modal");p(this,"content");p(this,"onCloseCallback");const t=this.attachShadow({mode:"open"});this.modal=document.createElement("div"),this.modal.classList.add("modal"),t.appendChild(this.modal),this.content=document.createElement("div"),this.content.classList.add("modal-content"),this.modal.appendChild(this.content),this.content.innerHTML=`
    ...
    `;const a=this.shadowRoot.getElementById("uname"),o=this.shadowRoot.getElementById("passwordContainer"),s=this.shadowRoot.getElementById("login"),d=this.shadowRoot.getElementById("requestAccess");let r=null;r=this.shadowRoot.getElementById("psw"),r.addEventListener("input",()=>{r.style.border=""}),g.getUserData().then(c=>{c&&r&&(r.value=c.password,a.value=c.firstName)}),a.addEventListener("input",async()=>{const c=a.value.trim().toLowerCase(),n=await C();c===""?(o.style.display="none",s.style.display="none",d.style.display="block"):n[c]?(o.style.display="block",s.style.display="block",d.style.display="none"):(o.style.display="none",s.style.display="none",d.style.display="block")}),d.style.display="block",d.addEventListener("click",()=>{window.open("https://forms.office.com/Pages/ResponsePage.aspx?id=J-soOqbWJUmXJZuWlVm4i-iWZheT5UVMtvugZuufuFtUQjI1TExGSjhGTFdRTlMxRlBXTFVPV1NLMy4u","_blank")}),s.onclick=async()=>{const c=a.value.trim().toLowerCase(),n=r==null?void 0:r.value.trim();try{await g.setUserData({email:c,password:n}),this.close()}catch(l){console.warn("Authentication failed:",l),l instanceof Error?alert(l.message):alert("An unknown error occurred"),r&&(r.style.border="2px solid red")}};const m=document.createElement("style");m.appendChild(document.createTextNode(`
        @keyframes slideDown {
          0% {
            opacity: 0;
